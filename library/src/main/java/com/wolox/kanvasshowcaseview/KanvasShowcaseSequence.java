...
public class KanvasShowcaseSequence implements IDetachedListener {

    PrefsManager mPrefsManager;
    Queue<KanvasShowcaseView> mShowcaseQueue;
    ...
    public KanvasShowcaseSequence(Activity activity) {
        mActivity = activity;
        mShowcaseQueue = new LinkedList<>();
    }

    public KanvasShowcaseSequence(Activity activity, String sequenceID) {
        this(activity);
        this.singleUse(sequenceID);
    }

    public KanvasShowcaseSequence addSequenceItem(View targetView, String content, String dismissText) {

        KanvasShowcaseView sequenceItem = new KanvasShowcaseView.Builder(mActivity)
    ...
    public KanvasShowcaseSequence addSequenceItem(KanvasShowcaseView sequenceItem) {
        mShowcaseQueue.add(sequenceItem);
        return this;
    }

    public KanvasShowcaseSequence singleUse(String sequenceID) {
        mSingleUse = true;
    ...
    private void showNextItem() {
        ...
        if (mShowcaseQueue.size() > 0 && !mActivity.isFinishing()) {
            KanvasShowcaseView sequenceItem = mShowcaseQueue.remove();
    ...
    @Override
    public void onShowcaseDetached(KanvasShowcaseView showcaseView, boolean wasDismissed) {
    ...
    public interface OnSequenceItemShownListener {
        void onShow(KanvasShowcaseView itemView, int position);
    }

    public interface OnSequenceItemDismissedListener {
        void onDismiss(KanvasShowcaseView itemView, int position);
    }
